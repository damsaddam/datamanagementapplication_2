            ...
            <div class="content row">
                <div class="col-lg-12">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover" id="guru">
                            <thead>
                                <tr>
                                    <th>No.</th>
                                    <th>Nomor Induk</th>
                            ...
                            <tbody>
                                ...
                                if ($_SERVER['REQUEST_METHOD'] == "POST") {
                                    ...
                                    if ($pencarian != '') {
                                        $sql = "SELECT * FROM tb_guru WHERE nama_guru LIKE '%$pencarian%' ORDER BY nama_guru, wali_kelas ASC";
                                        $query = $sql;
                                        $queryJml = $sql;
                                    } else {
                                        $query = "SELECT * FROM tb_guru ORDER BY nama_guru, wali_kelas ASC LIMIT $posisi, $batas";
                                        $queryJml = "SELECT * FROM tb_guru ORDER BY nama_guru, wali_kelas ASC";
                                        $no = $posisi + 1;
                                    }
                                } else {
                                    $query = "SELECT * FROM tb_guru ORDER BY nama_guru, wali_kelas ASC LIMIT $posisi, $batas";
                                ...
                                if (mysqli_num_rows($sql_guru) > 0) {
                                    while ($data = mysqli_fetch_array($sql_guru)) { ?>
                                        <tr>
                                            ...
                                            <td><?= $data['no_telp'] ?></td>
                                            <td class="config">
                                                <a href="edit.php?id=<?= $data['id_guru'] ?>" onclick="return confirm('Anda yakin ingin mengedit data ini?')"><i class="edit bi bi-pencil-square"></i></a>
                                                <a href="del.php?id=<?= $data['id_guru'] ?>" onclick="return confirm('Anda yakin ingin menghapus data ini?')"><i class="del bi bi-trash-fill"></i></a>
                                            </td>
                                        </tr>
                                <?php
                                    }
...
<script>
    $(document).ready(function() {
        $('#guru').DataTable({
            ...
            buttons: [{
                    extend: 'pdf',
                    title: 'DATA GURU SDN KALIBARU 09',
                    download: 'open',
